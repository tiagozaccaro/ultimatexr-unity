...
namespace UltimateXR.Locomotion.Teleport
{
    ...
    public partial class UxrTeleportLocomotion : UxrTeleportLocomotionBase
    {
        ...
        {
            base.OnDisable();
            UpdateTeleportState(false, false, false);
        }

        /// <summary>
        ///     Updates the arc if the avatar is non-local.
        /// </summary>
        private void Update()
        {
            if (Avatar.AvatarMode == UxrAvatarMode.UpdateExternally)
            {
                if (_lastSyncIsArcEnabled)
                {
        ...
        protected override void UpdateTeleportLocomotion()
        {
            ...
            if (_lastSyncIsArcEnabled != isArcEnabled || _lastSyncIsTargetEnabled != isTargetEnabled || _lastSyncIsValidTeleport != isValidTeleport)
            {
                UpdateTeleportState(isArcEnabled, isTargetEnabled, isValidTeleport);
            }
        }
        ...
        /// <summary>
        ///     Updates the state of elements in the teleport. This is mainly to synchronize the state for networking/replays.
        /// </summary>
        ...
        private void UpdateTeleportState(bool isArcEnabled, bool isTargetEnabled, bool isValidTeleport)
        {
            ...
            EndSyncMethod(SyncParams(isArcEnabled, isTargetEnabled, isValidTeleport));
        }
        ...
        private void ComputeCurrentArcTrajectory(out bool isArcEnabled, out bool isTargetEnabled, out bool isValidTeleport)
        {
            ...
            if (Mathf.Abs(angle) < AbsoluteMaxArcAngleThreshold && _arcWidth > 0.0f)
            {
                ...
                for (float time = 0.0f; time < endTime; time += deltaTime)
                {
                    ...
                    if (HasBlockingRaycastHit(UxrLocomotionRaycastPurpose.Targeting, point1, direction, distanceBetweenPoints, out RaycastHit hit))
                    {
                        endTime         = time + deltaTime * (hit.distance / distanceBetweenPoints);
                        hitSomething    = true;
                        isValidTeleport = NotifyDestinationRaycast(hit, UxrTeleportValidationChecks.Standard, out isTargetEnabled);
                        break;
                    }
                }
